...
namespace jgl
{
	Check_entry::Check_entry(jgl::String p_entry, bool p_state, Widget* p_parent) : Widget(p_parent)
	{
		...
		_entry.set_horizontal_alignment(Horizontal_alignment::centred);
		_entry.set_vertical_alignment(Vertical_alignment::centred);
	...
	void Check_entry::set_geometry_imp(Vector2 p_anchor, Vector2 p_area)
	{
		...
		_entry.set_anchor(Vector2(check_area.x, 0.0f) + _box.border() * 3);
