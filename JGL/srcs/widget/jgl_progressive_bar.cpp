...
namespace jgl
{
	...
	void Progressive_bar::render()
	{
		...
		fill_rectangle(jgl::Vector2(0, -1) + _box.border(), edited_size, _color, _layer, _viewport);
		if (_print_value == true)
		{
			...
			draw_centred_text(tmp_text, jgl::Vector2(0, -1) + _box.border() + size / 2, static_cast<uint32_t>(size.y / 1.3f), 0, 1.0f, jgl::text_color::black, jgl::text_style::normal, _layer, _viewport);
		}
	}
